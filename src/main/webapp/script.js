...
function mostrarPopUp(mensaje) {
    ...
    // Cierra el pop-up automáticamente después de 3 segundos
...
function actualizarFichas(montoGanado) {
	...
	document.getElementById("dinero").textContent = dinero.toFixed(2); 
}
...
btnReiniciar.addEventListener("click", () => {
	...
	mostrarPopUp("REINICIANDO JUEGO...");
...
function comprobarFilas() {
	...
	for (const fila of filas) {
		...
		if (Array.isArray(fila) && fila.includes(resultadoRuleta)) {
      		recibirFichas(36);
      		actualizarFichas();
      		mostrarPopUp(`Has acertado la fila. La ruleta ha sacado el número ${resultadoRuleta}`);
  	...
  	restarFicha(12);
  	actualizarFichas();
  	mostrarPopUp(`Has perdido. La ruleta ha sacado el ${resultadoRuleta}`);
...
function comprobarDocenas() {
	...
	for (const docena of docenas) {
		...
		if (Array.isArray(docena) && docena.includes(resultadoRuleta)) {
      		...
      		recibirFichas(36);
      		actualizarFichas();
      		mostrarPopUp(`Has acertado en la docena. La ruleta ha sacado el ${resultadoRuleta}`);
	...
	const opciones = [valorBtPa, valorBtIn];
		...
		for (const opcion of opciones) {
			...
			if (Array.isArray(opcion) && opcion.includes(resultadoRuleta)) {
	      		recibirFichas(24);
	      		actualizarFichas();
	      		mostrarPopUp(`Has acertado la ficha. La ruleta ha sacado el ${resultadoRuleta}`);
...
function jugadaSeleccionada() {
    mostrarPopUp(`Has seleccionado la ficha: ${Array.isArray(valorBoton) ? valorBoton.join(", ") : valorBoton}`);
}

// /////////////////////////////////////////////////////////////////////////////////////////////////////////////

// EventListener para cuando se pulsa el botón de jugar
btnJugar.addEventListener("click", () => {
  
	if (totalFichas < 1) {
    	mostrarPopUp("No tienes fichas suficientes para jugar. Por favor, reinicia el juego.");
    	return;
  	}

  	if (valorBoton == null) {
    	mostrarPopUp("¡Error! Debes seleccionar una ficha para poder jugar");
    	return;
  	}

  	// Llamada AJAX para obtener el número aleatorio
  	var xhr = new XMLHttpRequest();
  
  	xhr.open("GET", "NumeroAleatorioServlet");
  
  	xhr.onreadystatechange = function() {
		  
		if (xhr.readyState === 4 && xhr.status == 200 ){
			
			resultadoRuleta = parseInt(xhr.responseText);

        	mostrarPopUp(`Jugando la ficha... ${Array.isArray(valorBoton) ? valorBoton.join(", ") : valorBoton}`);
        	...
        	setTimeout(() => {
          		...
          		if (valorDocena1 || valorDocena2 || valorDocena3) {
      				...
      				if (totalFichas < 12) {
        				mostrarPopUp("No tienes suficientes fichas para jugar. Por favor, reinicia el juego.");
      				} else {
        				comprobarDocenas();
      				}
